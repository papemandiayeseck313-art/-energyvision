...
.auteur-badge {
    background: linear-gradient(90deg, #0d2137, #0a3d62);
...
with st.sidebar:
    ...
    <div style='text-align:center; padding: 10px 0 20px'>
        ...
        <h2 style='color:#1e90ff; margin:4px 0; font-family:Orbitron'>EnergyVision</h2>
    </div>
    """, unsafe_allow_html=True)

    st.markdown("""
    <div class='auteur-badge'>
        👤 <b>Pape Mandiaye Seck</b><br>
        L1 Big Data · Dakar Institute of Technology<br>
        Année 2025 / 2026
    </div>
    """, unsafe_allow_html=True)
...
def train_model(df):
    ...
    X = df2[['heure','jour_semaine','mois','annee','saison',
...
if page == "📊 Tableau de bord":
    ...
    with c1:
        st.markdown(f"""<div class='metric-box'>
            <p>Total mesures</p><h2>{df.shape[0]:,}</h2></div>""", unsafe_allow_html=True)
    with c2:
        st.markdown(f"""<div class='metric-box'>
            <p>Conso moyenne (kW)</p><h2>{df['Global_active_power'].mean():.3f}</h2></div>""", unsafe_allow_html=True)
    with c3:
        st.markdown(f"""<div class='metric-box'>
            <p>Conso max (kW)</p><h2>{df['Global_active_power'].max():.2f}</h2></div>""", unsafe_allow_html=True)
    with c4:
        st.markdown(f"""<div class='metric-box'>
            <p>Période</p><h2 style='font-size:1.1rem'>2006–2010</h2></div>""", unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)

    # Graphique 1 — journalier
    ...
    # Graphique 3 — par saison
    st.markdown("### 🌍 Consommation par saison")
    saison_map = {12:'Hiver',1:'Hiver',2:'Hiver',3:'Printemps',4:'Printemps',
                  5:'Printemps',6:'Été',7:'Été',8:'Été',9:'Automne',10:'Automne',11:'Automne'}
    ...
    bars = ax3.bar(saison_data.index, saison_data.values, color=colors)
...
# ══════════════════════════════════════════════════════════════
    ...
    with col3:
        jour  = st.slider("📆 Jour de la semaine", 0, 6, 0,
                          help="0=Lundi ... 6=Dimanche")

    saison_input = {12:0,1:0,2:0,3:1,4:1,5:1,6:2,7:2,8:2,9:3,10:3,11:3}[mois]
    input_data = pd.DataFrame(
        [[heure, jour, mois, 2009, saison_input, 241.0, 4.6, 1.0, 1.0, 6.0]],
        columns=['heure','jour_semaine','mois','annee','saison',
                 'Voltage','Global_intensity','Sub_metering_1',
                 'Sub_metering_2','Sub_metering_3'])
    ...
    st.markdown(f"""
    <div class='metric-box' style='max-width:400px; margin: 30px auto'>
        <p>⚡ Consommation prédite</p>
        <h2>{prediction:.3f} kW</h2>
        <p>Heure {heure}h · Mois {mois} · Jour {jour}</p>
    </div>
    """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════
    ...
    <p>Ce projet analyse et prédit la consommation électrique d'un foyer résidentiel
    à partir d'un jeu de données réel de <b>2 075 259 mesures</b> collectées entre
    2006 et 2010 (source : UCI Machine Learning Repository).</p>

    <h4 style='color:#00d4aa'>🎯 Objectifs</h4>
    <ul>
        <li>Analyser les tendances et patterns de consommation</li>
        <li>Identifier les pics de consommation par heure, jour et saison</li>
        <li>Prédire la consommation grâce au Machine Learning</li>
    </ul>

    <h4 style='color:#00d4aa'>🛠️ Technologies utilisées</h4>
    <ul>
        <li><b>Python</b> — Pandas, NumPy, Matplotlib</li>
        <li><b>SQL</b> — SQLite via SQLAlchemy</li>
        <li><b>Machine Learning</b> — Scikit-learn (Régression Linéaire, Random Forest), XGBoost</li>
        <li><b>Web App</b> — Streamlit</li>
    </ul>

    <h4 style='color:#00d4aa'>📊 Résultats des modèles</h4>
    <ul>
        <li>Régression Linéaire — R² : 0.9982</li>
        <li>XGBoost — R² : 0.9992</li>
        <li><b>Random Forest — R² : 0.9994 🏆 Meilleur modèle</b></li>
    </ul>

    <h4 style='color:#00d4aa'>👤 Auteur</h4>
    <p><b>Pape Mandiaye Seck</b><br>
    Étudiant en L1 Big Data<br>
    Dakar Institute of Technology · 2025/2026</p>
